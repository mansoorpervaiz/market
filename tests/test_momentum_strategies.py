...
class TestMomentumStrategies(unittest.TestCase):
    ...
    def test_top_breakout_strategy_with_stop_loss(self):
        """Test the TopBreakoutStrategy with trailing stop loss."""
        # Create data with clear breakout patterns and price movements for testing stop loss
        dates = pd.date_range(start='2023-01-01', end='2023-02-15', freq='D')
        num_days = len(dates)
        ...
        uptrend_segment = int(num_days * 0.2)   # 20% of days for uptrend
        pullback_segment = int(num_days * 0.2)  # 20% of days for pullback (to trigger stop loss)
        final_segment = num_days - initial_segment - breakout_segment - uptrend_segment - pullback_segment  # Remaining days
        ...
        # Create the strategy with trailing stop enabled
        ...
        if (signals['signal'] == Signal.BUY.value).any():
            ...
            first_buy_date = buy_signals.index[0]

            # Convert all dates to strings for comparison to avoid type issues
            date_strings = [d.strftime('%Y-%m-%d') for d in signals.index]
            first_buy_date_str = first_buy_date.strftime('%Y-%m-%d')

            # Find the index of the first buy date in our string list
            first_buy_idx = date_strings.index(first_buy_date_str)
